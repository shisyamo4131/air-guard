...
3. 配置管理で、配置されている従業員のセルにジャンプしたい（UI が決まらないので保留）
4. マスタデータを削除した後、一覧画面に遷移した際に削除したデータが一覧に表示されてしまう。 -> Vuex が参照している Realtime Database からの削除処理を Cloud Functions が担っているためのタイムラグ。
5. GInputSubmitMixin で、エラー発生時のアラート表示を止めたい。エラーメッセージ用プロパティを用意して、VAlert を使うようにするか？
6. 従業員選択コンポーネント（Autocomplete）で、在職中かどうかを選択できないか？ もしくは選択 UI を一新？

## 2024-11-xx - ver 0.7.0
