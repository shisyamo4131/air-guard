...
- 2024-12-26 - v18.12.0 -> v18.20.5 (AirGuard Client を Nuxt3 で実装するにあたって開発環境を共通化するために必要)
- 2024-10-20 - v18.11.0 -> v18.12.0

# Firebase Updates

- 2025-02-06 - v13.29.0 -> v13.30.0
...
### 配置管理での Placement クラスインスタンスの節約について

- リソースを節約するために配置データの存在するセルのみ実行しようとするとうまくいかない。制御が複雑になるため、特段の理由がない限りは現行のままでいくこと。

### マスタデータを削除した後、一覧画面に遷移した際に削除したデータがしばらく表示される

- Vuex が参照している Realtime Database からの削除処理を Cloud Functions が担っているためのタイムラグ。

## 課題・要望一覧

1. 配置表を開いている人が知りたい。（かなり面倒）
2. GInputSubmitMixin で、エラー発生時のアラート表示を止めたい。エラーメッセージ用プロパティを用意して、VAlert を使うようにするか？
3. ミルックスの請求書出力を作りたい？？？
...
- Vuex の従業員情報を取得し切る前に満了間近の雇用契約をループさせることで発生するエラーを回避

### 修正: ユーザー管理

- 従業員未指定でユーザーアカウントを作成できるように修正

### 修正: 配置表管理

- 各従業員の累積残業時間をタグに表示するように修正

### その他

- Node のバージョンを v18.20.5 にアップグレード
- 全体の配色を変更
- GTemplateIndex をリファクタリング。
- GTemplateDocumentsIndex を新規実装。伴って、Firestore ドキュメントを一覧表示、編集する画面のテンプレートを差し替え。
- EmployeeLeaveApplication を LeaveApplication に変更。
- 出力される HTML の lang が en になってしまっていたのを修正
- ボタンコンポーネントを整理
- その他軽微なバグを修正

## 2024-12-23 - ver 0.9.3

### 修正: Cloud Functions

- 非正規化データの同期処理 (Cloud Functions) をリファクタリング。

### 修正: GTemplate

- コンポーネントをリファクタリングし、不要なコンポーネントを削除。

## 2024-12-23 - ver 0.9.2

### 修正: 従業員管理

- 従属ドキュメントの非正規化データの同期処理を追加

### 追加: 従業員の健康診断情報管理機能

- 新規実装

## 2024-12-23 - ver 0.9.1

- 健康保険、厚生年金、雇用保険の管理機能完成

## 2024-12-20 - ver 0.9.0

### 新規実装

- 単一ドキュメントの購読とデータ提供を行うレンダーレスコンポーネント`ADocumentSubscriber`を実装
- 複数ドキュメントの購読とデータ提供を行うレンダーレスコンポーネント`ADocumentsSubscriber`を実装
- オブジェクトをリスト表示するためのコンポーネント`GListIterator`を実装

### 機能追加: 従業員管理

- 健康保険、厚生年金、雇用保険の情報を管理できる機能を追加（但し、UI については一考の余地あり）
