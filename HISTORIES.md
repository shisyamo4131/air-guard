...
2. 配置表を開いている人が知りたい。
3. 現場稼働予定に、休工フラグを追加して必要人数を強制的に 0 にする方が良い。
4. 現場並べ替えでダイアログが終了した時にスクロール位置が戻っていない。
5. 現場並べ替えコンポーネントを再利用可能コンポーネントにできそう。
6. 稼働予定管理で、siteOrder をリアルタイムリスナーで購読しないと複数人で使えない。
7. 月次売上の完成させたい

- 自社情報に「自社情報である」ことをフラグで持たせる必要がある。
- フラグを持たせたら売上集計の処理を修正する。

8. トップ画面を再構築

## 2024-11-XX - ver x.x.x
