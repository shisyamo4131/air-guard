...
3. マスタデータを削除した後、一覧画面に遷移した際に削除したデータが一覧に表示されてしまう。
   -> Vuex が参照している Realtime Database からの削除処理を Cloud Functions が担っているためのタイムラグ。
4. PlacementEmployeeCardV2, PlacementOutsourcerCardV2 を削除
5. 現場の取極めを削除する方法がない
...
- メンテナンスモード設定
- system.js のバージョンアップ　-> ver 0.3.0
- 完全デプロイ
- Firestore.Systems.System のバージョンアップ -> ver 0.3.0
- メンテナンスからインデックスを再構築
- メンテナンスモード解除
