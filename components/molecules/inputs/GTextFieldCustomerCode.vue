  ...
  >
    <template #prepend>
      <v-tooltip top>
        ...
        <span>
          取引先のCODEを登録すると、<br />MS-Access版AirGuardの取引先情報と同期されます。
        </span>
      </v-tooltip>
    </template>
  </a-text-field>
...
export default {
  ...
  computed: {
    ...
    hint() {
      if (!this.fetchedItem) return 'AirGuardの取引先CODEを入力してください。'
      return `${this.fetchedItem.name1}と一致しました。`
    },
  },
  watch: {
    value: {
  ...
  methods: {
    async fetchItem() {
      const docRef = doc(this.$firestore, `AirGuardCustomers/${this.value}`)
