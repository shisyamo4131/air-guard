...
export default {
  ...
  methods: {
    /**
     * コンポーネントの状態を編集終了にします。
     * - data.isEditing のウォッチャーによりコンポーネントの初期化処理が実行されます。
     */
    quitEditing() {
    ...
    async submit() {
      ...
      if (!this.isEditing) {
        this._setError(`Cannot submit because it is not being edited.`)
        return
      }

      // data.editItem が null の場合はエラーを出力して終了
      if (!this.editItem || typeof this.editItem !== 'object') {
        this._setError(`Cannot submit because editItem is not set.`)
        return
      }
    ...
    toRegist() {
      ...
      this._initializeEditItem(this.schema)
      this.isEditing = true
    },
    ...
    _cloneObject(obj) {
      if (obj === null || typeof obj !== 'object') {
        const message = `The argument must be an object. Received ${typeof obj}.`
    ...
    async _handleAddItem() {
      // インデックスのチェック
      if (this.editIndex !== -1) {
        this._setError(`Invalid index. index: ${this.editIndex}`)
        return
      }
      ...
      try {
        ...
        this._setError(`Failed to add item.`, err)
      }
    },
    ...
    async _handleModifyItem() {
      ...
      try {
        ...
        this._setError(`Failed to modify item.`, err)
      }
    },
    ...
    async _handleRemoveItem() {
      ...
      try {
        ...
        this._setError(`Failed to remove item.`, err)
      }
    },
    ...
    /**
     * 引数に指定されたオブジェクトで data.editItem を初期化します。
     * @param {Object} obj - 初期化に使用するオブジェクトです。
     */
    _initializeEditItem(obj = {}) {
      Object.entries(obj).forEach(([key, value]) => {
    ...
    _setError(message, payload) {
      ...
      this.err = payload?.message || message
    },
  },
