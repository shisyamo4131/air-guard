...
export default {
   ...
   * COMPUTED
   ***************************************************************************/
  components: { GTemplateDefault, GCheckbox, GPagination },
  ...
  methods: {
     ...
     *   AirGuard/Outsourcers/{code}/docIdに同期対象のFirestoreドキュメントidを設定します。
    ...
    async submit() {
      ...
      try {
        ...
        /* 同期設定対象データへの参照を取得 */
        const dbRef = ref(database, `AirGuard/Outsourcers/${code}`)
