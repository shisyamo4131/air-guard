 ...
 * ## Vuex.outsourcers.js
 *
 * Realtime Database から取得した `Outsourcers` データを管理するVuexです。
 *
 * - Realtime Database の `Outsourcers` に登録されているインデックスデータを管理します。
...
export const getters = {
  ...
  /**
   * 引数で指定された外注先が契約中であればtrueを返します。
   * それ以外はfalseを返します。
   * @param {string} docId 外注先のドキュメントid
...
export const actions = {
  ...
  async subscribe({ commit }) {
    const dbRef = ref(database, 'Outsourcers')
