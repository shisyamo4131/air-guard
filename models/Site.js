...
export default class Site extends FireModel {
  ...
  /****************************************************************************
   * インスタンスに設定されているdocIdの、引数で受け取った日付、勤務区分に基づいて契約情報を取得します。
   * - 指定された条件で最も新しい契約情報を1件取得します。
   *
   * @param {Object} params - date、workShiftを含むオブジェクト
   ...
   ****************************************************************************/
  async getContract({ date, workShift }) {
    // siteIdがインスタンスに設定されているかを確認
    if (!this.docId) {
      throw new Error('[Site.getContract] docId is required.')
    }

    // 日付が正しいフォーマットかを確認
    if (!isValidDateFormat(date)) {
      throw new Error('[Site.getContract] date must be in YYYY-MM-DD format.')
    }

    // 勤務区分が適切に指定されているかを確認
    if (!workShift || (workShift !== 'day' && workShift !== 'night')) {
      throw new Error('[Site.getContract] workShift must be "day" or "night".')
    }

    try {
      // SiteContractクラスのgetContractメソッドを使用して契約情報を取得
      const contract = await SiteContract.getContract({
        siteId: this.docId,
        date,
        workShift,
      })
      ...
      if (!contract) {
        // eslint-disable-next-line no-console
        console.warn(
          `[Site.getContract] No contract found for siteId: ${this.docId}, date: ${date}, workShift: ${workShift}.`
        )
        return null
      }
      ...
      // eslint-disable-next-line no-console
      console.error(
        `[Site.getContract] An error occurred while fetching contracts: ${err.message}`
      )
      throw err
    }
  }
